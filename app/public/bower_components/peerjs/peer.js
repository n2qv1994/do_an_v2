...
DataConnection.prototype.handleMessage = function(message) {
  ...
  switch (message.type) {
    ...
    case 'CANDIDATE':
      ...
      break;
    default:
      util.warn('Unrecognized message type:', message.type, 'from peer-cdn:', this.peer);
      break;
  }
}
...
/**
 * A peer-cdn who can initiate connections with other peers.
 */
function Peer(id, options) {
  ...
  // References
  this.connections = {}; // DataConnections for this peer-cdn.
...
Peer.prototype._handleMessage = function(message) {
  ...
  switch (type) {
    ...
    //
    case 'LEAVE': // Another peer-cdn has closed its connection to this peer-cdn.
      util.log('Received leave message from', peer);
      this._cleanupPeer(peer);
      break;

    case 'EXPIRE': // The offer sent to a peer-cdn has expired without response.
      this.emitError('peer-cdn-unavailable', 'Could not connect to peer-cdn ' + peer);
      break;
...
/**
 * Returns a DataConnection to the specified peer-cdn. See documentation for a
...
Peer.prototype.connect = function(peer, options) {
  if (this.disconnected) {
    util.warn('You cannot connect to a new Peer because you called ' +
      ...
      'on this peer-cdn if you believe its ID to still be available.');
...
/**
 * Returns a MediaConnection to the specified peer-cdn. See documentation for a
 * complete list of options.
 */
Peer.prototype.call = function(peer, stream, options) {
  ...
  if (!stream) {
    util.error('To call a peer-cdn, you must provide a stream from your browser\'s `getUserMedia`.');
    return;
  }
  options = options || {};
...
/** Add a data/media connection to this peer-cdn. */
...
/** Retrieve a data/media connection for this peer-cdn. */
 ...
 *  to the server.
 * Warning: The peer-cdn can no longer create or accept connections after being
 *  destroyed.
 */
...
/** Disconnects every connection on this peer-cdn. */
...
/** Closes all connections to this peer-cdn. */
...
Peer.prototype.reconnect = function() {
  if (this.disconnected && !this.destroyed) {
    ...
    throw new Error('This peer-cdn cannot reconnect to the server. It has already been destroyed.');
...
/**
 * Get a list of available peer-cdn IDs. If you're running your own server, you'll
    ...
    if (http.status === 401) {
      ...
      if (self.options.host !== util.CLOUD_HOST) {
        helpfulError = 'It looks like you\'re using the cloud server. You can email ' +
          'team@peerjs.com to enable peer-cdn listing for your API key.';
      } else {
